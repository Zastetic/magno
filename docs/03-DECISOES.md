...
| D26 | Freio por IP (anti-abuso) | **Janela deslizante em memória, sem dependência externa** | `server/limites.py`: regras por ação (cadastro 5/h, código 6/h, login 15/15min, agendamento 12/h) e um teto geral de 600 req/min por IP. O IP vem de `CF-Connecting-IP` (o serviço só escuta em 127.0.0.1, então o único caminho é o túnel) — `MAGNO_ATRAS_DE_PROXY=0` desliga essa confiança. Estourou: **429 com `Retry-After`** e evento `limite_estourado` na auditoria. Roda em memória porque o serviço usa um worker só; no dia que virar multi-worker, a decisão muda para Redis/tabela. |
| D27 | CAPTCHA do Cloudflare (Turnstile) | **Ligado por ambiente, com modo `log` antes de `on`** | `server/turnstile.py` + `web/turnstile.js` (widget invisível, sem mexer no layout). Protege cadastro, código por e-mail, login e agendamento. Sem chave configurada o site funciona igual (`modo: off`). `log` verifica de verdade, grava o resultado e **não bloqueia** — é o passo seguro para medir quantos visitantes reais passam antes de virar bloqueio. Se o Cloudflare não responder, libera (fail-open) e registra: queda de terceiro não derruba o login. |
| D28 | Grade de horários do site | **Calculada no servidor a cada consulta (`server/agenda.py`), servida por `GET /api/publica/disponibilidade`** | O site não guarda dias nem horas no HTML: a tira de dias sai de `de`/`ate` (contagem por dia) e os horários de `data` (slots em UTC, convertidos na tela com o `fuso` que a própria resposta traz). O `POST /api/bookings` valida com o **mesmo motor** (`agenda.conferir`) e grava em `BEGIN IMMEDIATE` (`agenda.marcar`), então marcar um horário o tira da grade de todo mundo na consulta seguinte — inclusive na tela de quem marcou, que recarrega depois de confirmar e ao voltar para a aba (≥ 20 s entre recargas, para não virar enxurrada de requests). O motivo quando não há vaga vem explicado (`loja_fechada`, `fora_da_janela`, `antecedencia_minima`, `dia_lotado`, `sem_espaco_no_dia`, `sem_profissional_habilitado`) em vez de tela vazia. Regra 7 do plano (mesmo cliente não fica com dois horários sobrepostos) entra aqui: `409 ja_tem_agendamento`, e o próprio site esconde esses slots do cliente logado. |
| D29 | App no celular | **PWA instalável (Chrome Android → "Instalar app"; iPhone → "Adicionar à Tela de Início"), sem APK na v1** | `web/manifest.webmanifest` + `web/sw.js` + `web/pwa.js` + ícones gerados por `scripts/gera_icones_pwa.py` (do SVG do poste, via Playwright — sem arte-fonte PNG). O service worker guarda só o **casco** (ícones, CSS, `offline.html`): `/api/*` e as telas logadas ficam de fora de propósito, porque horário é dado vivo e cache de agenda mentiria para o cliente. `sw.js` sai com `Cache-Control: no-cache` (senão o app fica preso numa versão antiga) e o manifest em rota própria, porque o `StaticFiles` não conhece `.webmanifest`. A instalabilidade é medida pelo próprio Chrome (`Page.getInstallabilityErrors` vazio) em `scripts/testa_pwa.py`. APK de verdade (TWA/Bubblewrap + `assetlinks.json`) fica para depois do cookie `HttpOnly`: com `sessionStorage`, cada abertura do app pediria login de novo. |
